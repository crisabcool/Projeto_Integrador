...
<div class="row">
	<div class="col-md-3">
		<button style="margin-bottom:10px" onclick="inserir()" type="button" class="btn btn-primary btn-flat btn-pri"><i class="fa fa-plus" aria-hidden="true"></i> Novo Servico</button>
	</div>
				...
				<div class="modal-body">
					...
					<div class="row">
						<div class="col-md-5">						
							<div class="form-group"> 
								<label>Veiculo</label> 
								<select class="form-control sel3" id="veiculo" name="veiculo" style="width:100%;" required> 

									<?php 
									$query = $pdo->query("SELECT * FROM veiculos ORDER BY marca asc");
									$res = $query->fetchAll(PDO::FETCH_ASSOC);
									$total_reg = @count($res);
									if($total_reg > 0){
										for($i=0; $i < $total_reg; $i++){
											foreach ($res[$i] as $key => $value){}
												echo '<option value="'.$res[$i]['id'].'">'.$res[$i]['marca'].'</option>';
										}
									}
									?>
		...
		<div class="modal-content">
			...
			<form method="post" id="form-servico">
				<div class="modal-body">
					...
					<input type="hidden" name="veiculo_agd" id="veiculo_agd"> 
